...
- Must-cover: routing heuristic (text_layer vs vision), bbox placeholder emission, dedup key matching + `force`, per-user job cap 429, SKIP LOCKED claim under two concurrent workers, source-file deletion at terminal state, sweeper, key encryption round-trip + masking, OAuth AS happy path (DCR → PKCE → token → `/mcp` auth), 402/429 upstream handling.
